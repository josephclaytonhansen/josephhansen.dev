  ...
  >
    ...
    >
      ...
      <p :class="pClass(brightness)">
        ...
        on the Internet. Let me help you make your website accessible to
        everyone.
      </p>
      <h4 :class="pClass(brightness)">
        Here's a real world example from a site I've seen. Which button is the
        "Submit" button?
      </h4>
      <p :class="pClass(brightness)">
        These buttons only use icons, with no alt text- so for users using a
        screen reader, they have no idea what the buttons do.
      </p>
      <p :class="pClass(brightness)">
        It doesn't stop there, though- for users with color blindness, the green
        and red buttons are indistinguishable.
      </p>
      <div class="flex w-full">
        <button
          :class="{
            'bg-emerald-600 text-slate-200': brightness >= 4,
            'bg-slate-500 text-slate-200': brightness == 3,
            'bg-orange-600 text-slate-800': brightness == 2,
      ...
      <div class="flex w-full pt-4 gap-2">
        <div class="w-6/12">
          <button
            class="rounded px-5 py-2 w-full"
            :class="testSuccessButtonClass"
          >
      ...
      <div class="w-full flex">
        <div class="w-6/12">
          <button
            class="text-xl font-semibold rounded px-5 py-2 flex align-middle"
            :class="{
              'bg-emerald-600 text-slate-200': brightness >= 4,
              'bg-slate-500 text-slate-200': brightness == 3,
              'bg-orange-600 text-slate-800': brightness == 2,
      ...
      <p :class="pClass(brightness)">
        These buttons have screen reader labels and don't rely on color to
        indicate which is which.
      </p>

      <p :class="pClass(brightness)">
        Changes like these may seem small, but they make a
        <em>huge</em> difference for the usability of your site. Let me help you
        be in the 2%.
      </p>
    </div>
